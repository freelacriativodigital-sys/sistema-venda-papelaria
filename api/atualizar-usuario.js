  ...
  try {
    ...
    if (listError) {
      return res.status(400).json({
        error: listError.message || 'Erro ao buscar usuário no Authentication.',
      });
    }

    const authUser = (usersData?.users || []).find(
      (user) => String(user.email || '').trim().toLowerCase() === emailAtualLimpo
    );
    ...
    const payloadUpdateAuth = {
      email: novoEmailLimpo,
      email_confirm: true,
    };

    if (novaSenhaLimpa) {
      payloadUpdateAuth.password = novaSenhaLimpa;
    }
    ...
    if (updateAuthError) {
      return res.status(400).json({
        error: updateAuthError.message || 'Erro ao atualizar usuário no Authentication.',
      });
    }
    ...
    if (updateTableError) {
      return res.status(400).json({
        error: updateTableError.message || 'Erro ao atualizar perfil no banco.',
      });
    }
